...
public class ImageUtils {
  ...
  private static int[] resizeToMaxSize(int srcWidth, int srcHeight, int desiredWidth,
      int desiredHeight) {
    int[] size = new int[2];
    if (desiredWidth <= 0) {
  ...
  private static boolean checkBitmap(Bitmap bitmap) {
    if (bitmap == null) {
      L.e("ImageUtils", "原图Bitmap为空了");
      return false;
    }

    if (bitmap.getWidth() <= 0 || bitmap.getHeight() <= 0) {
      L.e("ImageUtils", "原图Bitmap大小为0");
      return false;
    }
    return true;
  }
  ...
  private static boolean checkSize(int desiredWidth, int desiredHeight) {
    if (desiredWidth <= 0 || desiredHeight <= 0) {
      L.e("ImageUtils", "请求Bitmap的宽高参数必须大于0");
      return false;
    }
    return true;
  }
   ...
   */
  @SuppressWarnings("deprecation") public static Drawable bitmapToDrawable(Bitmap bitmap) {
   ...
   */
  @SuppressWarnings("deprecation") public static Drawable view2Drawable(View view) {
    BitmapDrawable mBitmapDrawable = null;
    try {
    ...
    if (bitmap != null) {
      try {
        if (!bitmap.isRecycled()) {
          L.d("ImageUtils", "Bitmap释放" + bitmap.toString());
          bitmap.recycle();
        }
      } catch (Exception e) {
      }
